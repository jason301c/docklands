...
import { getDocklandsUrl } from "@dokploy/server/services/admin";
import {
...
export const serverSetup = async (
	...
	try {
		...
		if (IS_CLOUD) {
			...
			const baseUrl = await getDocklandsUrl();
...
if [ -x "$(command -v snap)" ]; then
    ...
    if [ "$SNAP_DOCKER_INSTALLED" = "true" ]; then
        ...
        echo "   Please note that Docklands does not support Docker installed via snap."
