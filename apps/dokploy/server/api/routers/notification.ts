...
export const notificationRouter = createTRPCRouter({
	...
	testTelegramConnection: withPermission("notification", "create")
		...
		.mutation(async ({ input }) => {
			try {
				await sendTelegramNotification(input, "Hi, From Docklands 👋");
				return true;
			} catch (error) {
				throw new TRPCError({
					code: "BAD_REQUEST",
		...
		.mutation(async ({ input }) => {
			try {
				...
				await sendDiscordNotification(input, {
					...
					description: decorate(">", "Hi, From Docklands 👋"),
					color: 0xf3f7f4,
				});
		...
		.mutation(async ({ input }) => {
			try {
				await sendNtfyNotification(
					...
					"Test Notification",
					"",
					"view, visit Docklands on Github, https://github.com/dokploy/dokploy, clear=true;",
					"Hi, From Docklands 👋",
				);
				return true;
			} catch (error) {
		...
		.mutation(async ({ input }) => {
			try {
				await sendMattermostNotification(input, {
					text: "Hi, From Docklands 👋",
					channel: input.channel,
					username: input.username || "Docklands Bot",
				});
				return true;
			} catch (error) {
				throw new TRPCError({
		...
		.mutation(async ({ input }) => {
			try {
				await sendTeamsNotification(input, {
					title: "🤚 Test Notification",
					facts: [{ name: "Message", value: "Hi, From Docklands 👋" }],
				});
				return true;
			} catch (error) {
				throw new TRPCError({
