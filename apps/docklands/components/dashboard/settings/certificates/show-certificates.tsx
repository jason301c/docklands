...
export const ShowCertificates = () => {
	...
	const certificatesQuery = api.certificates.all.useQuery();
	const { refetch } = certificatesQuery;
	...
	return (
		<SectionCard title="Certificates" contentClassName="space-y-2">
			<AlertBlock type="warning">
				...
				certificates can break ingress and prevent access to your applications.
			</AlertBlock>
			<QueryState
				...
				empty={
					<EmptyState
						icon={ShieldCheck}
						title="You don't have any certificates created"
						action={
							permissions?.certificate.create ? <HandleCertificate /> : null
						}
					/>
				}
			>
