...
// Legacy environment list UI kept temporarily while the canvas workspace absorbs
...
type Environment = RouterOutputs["environment"]["one"];
type EnvironmentServiceRecord = Record<string, any>;
...
export const extractServicesFromEnvironment = (
	...
	const applications: Services[] =
		environment.applications?.map((item: EnvironmentServiceRecord) => {
	...
	const mariadb: Services[] =
		environment.mariadb?.map((item: EnvironmentServiceRecord) => ({
			name: item.name,
			type: "mariadb",
			id: item.mariadbId,
			createdAt: item.createdAt,
	...
	const postgres: Services[] =
		environment.postgres?.map((item: EnvironmentServiceRecord) => ({
			name: item.name,
			type: "postgres",
			id: item.postgresId,
			createdAt: item.createdAt,
	...
	const mongo: Services[] =
		environment.mongo?.map((item: EnvironmentServiceRecord) => ({
			name: item.name,
			type: "mongo",
			id: item.mongoId,
			createdAt: item.createdAt,
	...
	const redis: Services[] =
		environment.redis?.map((item: EnvironmentServiceRecord) => ({
			name: item.name,
			type: "redis",
			id: item.redisId,
			createdAt: item.createdAt,
	...
	const mysql: Services[] =
		environment.mysql?.map((item: EnvironmentServiceRecord) => ({
			name: item.name,
			type: "mysql",
			id: item.mysqlId,
			createdAt: item.createdAt,
	...
	const compose: Services[] =
		environment.compose?.map((item: EnvironmentServiceRecord) => {
			// Get the most recent deployment date
			let lastDeployDate: Date | null = null;
			const deployments = (item as any).deployments;
			if (deployments && deployments.length > 0) {
	...
	const libsql: Services[] =
		environment.libsql?.map((item: EnvironmentServiceRecord) => ({
			name: item.name,
			type: "libsql",
			id: item.libsqlId,
			createdAt: item.createdAt,
...
const EnvironmentPage = (props: {
	...
	// Check if there are services without a server (Docklands server)
									...
									<div className="flex flex-col gap-2 lg:flex-row lg:gap-4 lg:items-center">
										...
										{(availableServers.length > 0 ||
											...
											<Select
												...
												onValueChange={(value) =>
													value !== null && setSelectedServerId(value as never)
												}
											>
												<></>
												<>
