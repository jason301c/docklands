	...
	const settings: ContainerCreateOptions = {
		...
		HostConfig: {
			...
			Binds: [
				...
				`${DYNAMIC_TRAEFIK_PATH}:/etc/docklands/traefik/dynamic`,
			...
			ContainerSpec: {
				...
				Mounts: [
					...
					},
					{
						...
						Target: "/etc/docklands/traefik/dynamic",
					},
					{
						Type: "bind",
...
export const getDefaultTraefikConfig = () => {
	const configObject: MainTraefikConfig = {
		...
		providers: {
			...
			file: {
				directory: "/etc/docklands/traefik/dynamic",
				watch: true,
			},
		},
		entryPoints: {
			...
			certificatesResolvers: {
				letsencrypt: {
					acme: {
						...
						storage: "/etc/docklands/traefik/dynamic/acme.json",
						httpChallenge: {
							entryPoint: "web",
						},
					},
