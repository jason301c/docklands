...
- UI source for ingress/runtime settings should stay under product-named modules such as `settings/ingress-runtime`, `settings/runtime/terminal`, and `container-runtime/*`; backend compatibility APIs like `getWebServerSettings` can remain until a dedicated data migration renames them.
