...
- Keep the custom Next server on Turbopack. `server/server.ts` passes `turbopack: true`, and app scripts should stay on plain `next build` without Webpack flags or custom Webpack config.
